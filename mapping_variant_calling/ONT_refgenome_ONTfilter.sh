...
OUTPUT_DIR=~/Sequencing_Combined/complete_mapping
...
minimap2 -ax map-ont --secondary=no -N 1 --min-chain-score 1000 \
    ...
    | samtools sort -o $OUTPUT_DIR/LVGU_longread_ont_filtered.bam

# 4. Index and cleanup
samtools index $OUTPUT_DIR/LVGU_longread_ont_filtered.bam
rm temp_*.fastq.gz

echo "Done. Ready for ANGSD with ONT-filtered reference."
samtools flagstat $OUTPUT_DIR/LVGU_longread_ont_filtered.bam
